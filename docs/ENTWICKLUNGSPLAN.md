...
> **Stand der Umsetzung:** Charakter „AJ" + 7 Clips (idle, run, sprint, jump, fall, roll, land) als FBX direkt geladen — kein GLB-Zwischenschritt. `src/core/AssetLoader.ts` (Lade-Overlay, Root-Motion-Strip für Nicht-In-Place-Clips), `src/player/PlayerAnimator.ts` (FSM→Clip mit crossFade 0.15 s, Roll als One-Shot). Kapsel unsichtbar, **F4** blendet sie ein; `?nochar=1` überspringt den ~17-MB-Download (genutzt von den Physik-Smoke-Tests, neuer Test `smoke-char` prüft den Ladepfad). **Offene Nacharbeit:** eigene Clips für HANG/BALANCE/SWING/WALLRUN/VAULT (zeigen aktuell Fallbacks — Erweiterungs-Set unten einfach nachladen und hochladen); FBX→GLB-Konvertierung als Teil von Task 28 (Downloadgröße).
