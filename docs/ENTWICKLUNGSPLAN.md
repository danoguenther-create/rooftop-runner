...
- **Spins 180/360 (Task 15b):** A/D in der Luft drehen um die Hochachse, gezählt in 180°-Schritten *(ursprünglich Q/E; seit dem Steuerungs-Umbau 2026-07-10 sind A/D kontextabhängig: am Boden drehen, in der Luft Spin)*. Nach der Landung läuft der Spieler in Kamerarichtung weiter — ein Spin verhunzt nicht die Steuerung. Mit Flips kombinierbar („360 Frontflip"), zählt als eigener Trick für den Multiplikator.
...
- **Tasten** *(Stand Splitscreen-Umbau 2026-07-10 — spielbar ganz ohne Maus; Keymaps in `src/core/Input.ts`)*:
  - **Spieler 1:** W/S vor/zurück · A/D **drehen** (auf der Rail: Ausgleichen, im Hang: Hangeln) · **Flips = Bewegungstaste in der Luft erneut drücken** (W Front, S Back, A/D Side; nochmal = Double/Triple; im Solo-Modus lösen auch die Pfeiltasten Flips aus) · Q/E Spin 180°-Schritte · Leertaste Sprung (kontextabhängig: Wall-Jump, Rail-Absprung, Swing-Release) · Shift Sprint · Strg/C Roll, in der Luft gehalten = Diveroll · R Respawn · Esc Pause · Maus-Kamera optional.
  - **Spieler 2 (Splitscreen):** Pfeiltasten vor/zurück/drehen · Flips = Pfeiltaste in der Luft erneut · Komma/Punkt Spin · Enter Sprung · Shift-Rechts Sprint · Strg-Rechts Roll/Dive · Backspace Respawn.
  - Vault und Ledge-Grab triggern automatisch (kein Extra-Knopf — reduziert Frust); im HANG: vor/Space hochziehen, zurück loslassen, seitlich hangeln. **Spawn-Schutz:** Lufttricks sind erst nach dem ersten Bodenkontakt scharf (sonst löst die Lauftaste während des Spawn-Falls einen Flip aus).
- **Splitscreen-Duell (2026-07-10):** `?mode=split` bzw. Startmenü-Buttons — zwei Spieler an einer Tastatur, Links/Rechts-Split (Scissor-Rendering, eigene FollowCamera + eigener EventBus/Score/HUD je Spieler, Spieler 2 mit blau getöntem Charakter-Klon via SkeletonUtils). Im Duell bewusst deaktiviert (V1): Collectibles, Time Trial, Missionen, Marker-Zonen — reiner Trick-Score-Vergleich. Die Spieler kollidieren physisch miteinander.
...
### Task 21 — Charakter + Animationen **[OPUS]** ✅ *(umgesetzt 2026-07-10)*

> **Stand der Umsetzung:** Charakter „AJ" + 12 Clips als FBX direkt geladen — kein GLB-Zwischenschritt. `src/core/AssetLoader.ts` (Lade-Overlay, Root-Motion-Strip für Nicht-In-Place-Clips, STRIP_Y für Bogen-Clips wie Wall Run), `src/player/PlayerAnimator.ts` (FSM→Clip mit crossFade 0.15 s, One-Shots, CLIP_START_S-Offsets per Hips-Kurve vermessen). Belegung: idle/run/sprint (Tempo), jump bzw. running-jump (Absprung mit/ohne Anlauf), fall, wallclimb (vertikaler Wandlauf), wallrun, hang (HANG **und** SWING), land (Bail), roll (Landeroll), landing-roll (Diveroll). Kapsel unsichtbar, **F4** blendet sie ein; `?nochar=1` überspringt den Download (Physik-Smoke-Tests; `smoke-char` prüft den Ladepfad). Ungenutzt im Ordner: Flip-Clips (Flips rotieren prozedural — Mixamo-Flips enthalten Anlauf + feste Einzelrotation, inkompatibel mit Double/Triple), big-jump, sprint-alt, jumping-down, swing (Run And Swing — Pendel kommt aus unserer Physik). Seit 2026-07-10 abends zusätzlich: balance (Catwalk Walk, BALANCE beim Gehen), vault (VAULT), sprint-roll (flache Landeroll <4 m; Falling To Roll übernimmt hohe Stürze). **Wichtige Falle behoben:** Mixamo nummeriert das Rig je Export zufällig (mixamorig9Hips …) — der AssetLoader normalisiert alle Track-Namen auf das Modell-Skelett, sonst bindet der Mixer nichts (T-Pose). **Nacharbeit:** FBX→GLB als Teil von Task 28; Wall-Run-Clip ist einseitig (lehnt immer zur selben Seite — ggf. später spiegeln).
